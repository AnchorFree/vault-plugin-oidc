...
func (b *backend) pathLogin(ctx context.Context,
	...
	tokenUsername, tokenGroups, tokenStruct, err := b.validateAndExtractClaims(ctx, config, token)
	if err != nil {
	...
	user, err := b.User(ctx, req.Storage, tokenUsername)
