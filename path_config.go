...
func (b *backend) oidcProviderForConfig(ctx context.Context, conf *ConfigEntry) (*oidc.Provider, error) {
...
func (b *backend) validateAndExtractClaims(ctx context.Context, cfg *ConfigEntry, rawToken string) (userName string, groups []string, token *oidc.IDToken, err error) {
	provider, err := b.oidcProviderForConfig(ctx, cfg)
	if err != nil {
